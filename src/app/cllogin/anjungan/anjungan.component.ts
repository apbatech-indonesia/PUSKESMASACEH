...
salah:any;
  ...
  lg(){
    ...
    this.authService.postData(this.formGroup.value, "login").then((result) =>{
...
console.log(this.nomorasuransi,noasuransix)
    ...
    this.authService.tmpbpjs(this.nomorasuransi,noasuransix)
    .subscribe(
      data => {
                    ...
                    if(data.length){
                      ...
                      let body ={
                        ...
                        "hp":this.nohp,"kdpoli":this.kdpoli,"kddokter":this.kddokter,"kelas":'1',
                           "tgldaftar":this.tgllahir,"kostumer":this.kdkostumer,"kdkostumer":this.kdasuransi,
                           "noasuransi":nokartu,
                      ...
                      this.toastr.error("Pasien / No Kartu belum terdaftar di fakses silahkan daftar melalui daftar rawat jalan")
...
// })
  ...
  kbpjsn(){
    this.toastr.error('SEDANG PENGEMBANGAN SILAHKAN DAFTAR DI BAGIAN PENDAFTARAAN', 'Eror');
  ...
  cetaknoantrian(notransaksi){
    ...
    var redirectWindow = window.open(this.URLINVOICE+'clenic/report/cetakantrian.php?kdcabang='+this.kdcabang+'&notransaksi='+notransaksi, '_blank','location=no,toolbar=no,height=570,width=500,scrollbars=yes,status=yes');
...
cekkepsertaan(content) {
  ...
  this.authService.tmpbpjs(this.nomorasuransi, noasuransix).subscribe((data) => {
    if (data) {
      console.log(data.metaData.code);
      if (data.metaData.code == 200) {
