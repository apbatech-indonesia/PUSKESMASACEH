...
export class mjadwalComponent implements OnInit {
  ...
  tmpjadwal() {
    this.authService.jadwaldokter(this.kdcabang).subscribe(
      (data) => {
  ...
  simpan() {
    let body = {
      ...
      stssimpan: "1",
      kodeantrian: this.kdantrian,
    };

    this.authService.simpanjadwal(body).subscribe((response) => {
      if (response) {
  ...
  non(a, b) {
    ...
    this.authService
      .cekjadwalv2(this.kdpolibpjs, a.target.value)
      .subscribe((data) => {
