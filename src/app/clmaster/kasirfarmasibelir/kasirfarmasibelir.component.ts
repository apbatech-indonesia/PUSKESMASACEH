...
@Component({
  selector: 'app-kasirfarmasibelir',
  templateUrl: './kasirfarmasibelir.component.html',
  styles: [
...
export class kasirfarmasibelirComponent implements OnInit {
...
this.tsupshow = false;
    ...
    ubahqty(a,kdobat,nomor,HNA,QTY,DISCRP,DISCPERSEN,DISCPERSENR,DISCRPR,QTYR){



var y =      parseInt(a.target.value)
var x =      parseInt(QTY)
console.log(y,x)
if( y > x){
    this.toastr.error('', 'Retur Lebih besar dari stok', {
      timeOut: 2000,
    });
}else{
      ...
      "kduser":this.username,"stssimpan":'7',"HNA":HNA,"QTY":QTY,
      "discrp":DISCRP,"discpersen":DISCPERSEN,"DISCPERSENR":DISCPERSENR,
      "DISCRPR":DISCRPR,"suplier":this.kdsuplier,"QTYR":QTYR}
...
.subscribe(response => {
  ...
  if(response ){
    ...
    ubahdiscpersen(a,kdobat,nomor,HNA,QTY,DISCRP,QTYR){
    ...
    ubahdiscrp(a,kdobat,nomor,HNA,QTY,DISCPERSEN,QTYR,DISCPERSENR){
      console.log(a.target.value)
    ...
    hapusall(){
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          ...
          let body={"nofakturbeli":this.nofakturbeli,"NOLPB":this.nolpb,"kdgudang":this.kdgudang,
        "kdcabang":this.kdcabang,"kdklinik":this.kdklinik,"noretur":this.noretur,
          "kduser":this.username,"stssimpan":'11'}
  ...
  ktglrad(){
    this.authService.caripembelianr(this.kdcabang,this.tglp,this.caripas,'')
    .subscribe(
      data => {
  ...
  this.tbelir = data;
    ...
    caripass(a){
      this.authService.caripembelianr(this.kdcabang,this.tglp,this.caripas,a.target.value)
      .subscribe(
        data => {
          this.totalpasenlist = data.length
    this.tbeli = data;
...
simpanbeli(){
  ...
  let body={
    ...
    "nofakturpajak":this.nofakturpajak,"tglfaktur":this.tglpx,"tgljatuhtempo":this.tglpxx,
    "kduser":this.username,"kdcabang":this.kdcabang,"stssimpan":'10',"kdklinik":this.kdklinik,"NOLPB" : this.nolpb,
    "JMLPPN":this.jmlppn,"PPN":this.ppnobat,
    "TOTAL":this.totalsetelahppn,"ADM":this.adm,"NETTO":this.totalakhir,
    "keterangan":this.keterangan,"kdgudang":this.kdgudang,"totalsebelumpajakr":this.totalsebelumpajakr,"jmlppnr":this.jmlppnr,
    "totalsetelahppnr":this.totalsetelahppnr
  }
...
setTimeout(() => {
    this.trxpbeli()
    this.noretur = response.metadata.message;
}, 150);
...
noretur:any;
pilihpasienr(NOFAKTUR,KDSUPPLIER,SYSTEMBAYAR,KDGUDANG,
  FAKTURPAJAK,NOLPB,TGLFAKTUR,TGLJATUHTEMPO,KETERANGAN,PPN,JMLPPN,TOTAL,ADM,NETTO,nama,NORETUR,TGLRETUR){
...
setTimeout(() => {
  ...
  cetakfaktur(){
    var redirectWindow = window.open(this.URLINVOICE+'clenic/report/fakturretur.php?nofaktur='+this.nofakturbeli+'&kdcabang='+this.kdcabang+'&noretur='+this.noretur, '_blank','location=no,toolbar=no,height=1000,width=1000,scrollbars=yes,status=yes');
    redirectWindow.location;
 }

 cetakfakturp(){
  var redirectWindow = window.open(this.URLINVOICE+'clenic/report/returpriodik.php?tgldari='+this.tgldari+'&tglsampai='+this.tglsampai+'&kdcabang='+this.kdcabang, '_blank','location=no,toolbar=no,height=1000,width=1000,scrollbars=yes,status=yes');
  redirectWindow.location;
}
