...
export class kajianperawatComponent implements OnInit {
  ...
  tmpriwayatno() {
    this.authService.rwtkajianperawatawal(this.norm, "2").subscribe((data) => {
      this.listrwt = data;
    });
  }
  ...
  tmpriwayatnotransaki() {
    this.authService
      .rwtkajianperawatawal(this.notrans, "1")
      .subscribe((data) => {
        for (let x of data) {
  ...
  users = [
    ...
    "Kurang Bulan",
    "BB Lahir...",
    "Panjang Badang lahir ...",
  ];
  listanalisis = [
    "Bersihkan Jalan Nafas tidak efektif",
    "perubahan nutrisi kurang/lebih cairan",
    ...
    "cemas",
    "hipertermi",
    "Menganjurkan pasien minum obat teratur",
    "Mengajurkan untuk makan teratur",
    "mengajurkan untuk minum hangat",
    "Mengajurkan untuk tidak minum dingin",
    "mengajurkan pasien cukup istirahat",
    "mengajurkan pasien kontrol teratur",
    "mengajurkan untuk membatasi aktivitas",
  ];
  ...
  simpan() {
    ...
    this.authService.simpankajian(bodyx).subscribe((response) => {
      console.log(response);

      if (response.kode == 200) {
        setTimeout(() => {
          this.authService.simpankajian(body).subscribe((response) => {
            if (response.kode == 200) {
              setTimeout(() => {}, 250);
