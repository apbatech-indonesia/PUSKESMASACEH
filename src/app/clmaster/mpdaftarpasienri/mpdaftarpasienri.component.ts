...
export class mpdaftarpasienriComponent implements OnInit {
  ...
  caripassd(a) {
    this.authService.t_listpasienri(this.kdcabang, a.target.value).subscribe(
      (data) => {
        this.tmppas = data;
  ...
  pilihkelas(a) {
    this.authService.t_kamarkelas(this.kdcabang, "", a).subscribe(
      (data) => {
        this.tkamar = data;
      },
      (Error) => {
  ...
  tmppuser() {
    this.authService.t_spesialisasi(this.kdcabang, "").subscribe(
      (data) => {
    ...
    this.authService.t_masterkelaskamar(this.kdcabang, "").subscribe(
      (data) => {
        this.tkelas = data;
      },
      (Error) => {
  ...
  hapusdaftar(
    ...
    if (notransaksi === "") {
      this.toastr.error(
        "data tidak bisa di edit karena pasien tersebut belum terdaftar",
        "Eror"
      );
    } else {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.value) {
            ...
            this.authService.simpandaftarri(body).subscribe((response) => {
              if (response) {
                if (response === 201) {
                  this.toastr.error(
                    "Data Tidak Bisa di hapus karena sudah ada transaksi",
                    "Eror"
                  );
                } else {
  ...
  pilihklinik(a) {
    ...
    this.authService.t_spesialisasimaping(this.kdcabang, a).subscribe(
      (data) => {
        this.tdokter = data;
  ...
  simpan() {
    ...
    // console.log(bodyx)
    this.authService.simpandaftarrj(bodyx).subscribe((response) => {
      if (response.length) {
        ...
        if (result.value) {
          ...
          this.authService.hapustrx(body).subscribe((response) => {
            if (response) {
              ...
              if (this.dashx === "BPJS") {
                this.toastr.success(
                  "Berhasil Hapus ,Untuk Di pCare Silahkan Hapus lewat Portal",
                  "-",
                  {
                    timeOut: 2000,
                  }
  ...
  daftarbpjs(content) {
    if (this.pcare <= 0) {
      ...
      this.authService.tmpbpjs(this.noasuransi, this.kdcabang).subscribe(
        (data) => {
          if (data) {
            console.log(data.metaData.code);
            if (data.metaData.code === 200) {
