...
.subscribe(
  data => {
    ...
    this.urlposter = Array.prototype.map.call(data,s=>s.urlposter).toString();
    this.urlvidio = Array.prototype.map.call(data,s=>s.urlvideo).toString();
},
  Error => {
...
this.authService.dokterperpoli('003',this.kduser)
.subscribe(
  data => {
  for(let x of data){
...
// console.log('sadasdasdsa')
  ...
  if('Antrian' === userData['userData']['hakakses']){
        //  this.pasienNotificationl(pesan)
        // alert(pesan)
        this.tmptotal()
        this.terbilangx(pesan)
        this.Nomor = pesan;
...
setTimeout(() => {
  ...
  audio.src='https://drnanik.digimedik.com/clenic/sound/' + strings[0]+'.wav';
  audio.play();
  
  audio.onended = function() {
  if(index < strings.length){
      audio.src='https://drnanik.digimedik.com/clenic/sound/' + strings[index]+'.wav';
      audio.play();
      index++;
  }
  };
