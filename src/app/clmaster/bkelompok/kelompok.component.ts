...
export class kelompokComponent implements OnInit {
	...
	onLoadDataKegiatan(date: string): any {
		this.authService.getAllBpjsKegiatan(this.pipe.transform(date, 'dd-MM-yyyy')).subscribe(
			(data: any) => {
				if (!data.success) {
					this.isProccessFailed = true
				} else {
	...
	showSavingDialog(savingType: string = "kegiatan") {
		...
		let addPesertaDialog: HTMLDialogElement | null = document.getElementById("dialogPeserta") as HTMLDialogElement;
	...
	doSavePeserta() {
		let dialog: HTMLDialogElement | null = document.getElementById("dialogPeserta") as HTMLDialogElement;
		dialog.close()
		Swal.fire({
	...
	showDialogPeserta(kegiatan: any) {
		let dialogPeserta: HTMLDialogElement | null = document.getElementById("dialogPeserta") as HTMLDialogElement;
			...
			if (result.isConfirmed) {
				...
				this.authService.deleteBpjsKegiatan(kegiatan.eduId).subscribe(
					(data: any) => {
						Swal.close()
						if (data.success) {
							const msg = data.message && data.message != '' ? data.message : 'Berhasil Menghapus Data'
							this.toastr.success(msg, 'Sukses', {
								timeOut: 2000,
							})
					...
					err => {
						Swal.close()
						const msg = err.error.err && err.error.err != '' ? err.error.err : 'Gagal Menghapus Data'
						this.toastr.error(msg, 'Error', {
							timeOut: 2000,
						})
					}
	...
	deletePesertaDialog(peserta: any) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				Swal.fire('Mohon tunggu!')
				Swal.showLoading()
				this.authService.deleteBpjsPesertaKegiatan(peserta.eduId, peserta.noKartu).subscribe(
					(data: any) => {
						Swal.close()
						if (data.success) {
							const msg = data.message && data.message != '' ? data.message : 'Berhasil Menghapus Data'
