...
@Component({
  selector: 'app-mutasifarmasiout',
  templateUrl: './mutasifarmasiout.component.html',
  styles: [
...
export class mutasifarmasioutComponent implements OnInit {
...
trxmutasix(){
  this.authService.trxmutasiout(this.kdcabang,this.nomutasi)
  .subscribe(
    data => {
...
onqty(a){
  if(this.nomutasi === ''){
    ...
    this.authService.simpanmutasiout(body)
    .subscribe(response => {
...
.subscribe(response => {
    ...
    hapusobat(nomutasi,nomor,kdobat,qty,obat,standart){
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          ...
          let body={"nomutasi":nomutasi,"nomor":nomor,"kdobat":kdobat,"qty":qty,"kdcabang":this.kdcabang,"kdklinik":this.kdklinik,"stssimpan":'3',"kdsuplier":this.kdsuplier,"kdgudang":this.kdgudang}
  ...
  this.tbeli = data;
    ...
    caripas='1';
tmpbeli(){
  this.authService.listmutasiout(this.kdcabang,this.tglp,this.caripas,'')
  .subscribe(
    data => {
      this.totalpasenlist = data.length
this.tbeli = data;
...
pilihpasien(mutasi,tgl,kdsuplier,kdgudang,keterangan,nama)
  {


    this.nomutasi = mutasi;
    this.tglp = tgl;
    this.kdsuplier = kdsuplier;
