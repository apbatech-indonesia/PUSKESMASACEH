...
@Component({
  selector: 'app-kasirri',
  templateUrl: './kasirri.component.html',
  styles: [
...
export class kasirriComponent implements OnInit {
...
this.tglpp = this.datepipe.transform(this.myDate, 'yyyy-MM-dd');
  }
...
this.kdbapak = a.node.key
  this.authService.tarifdetail(this.kdcabang,'2','',a.node.key,'RI')
  .subscribe(
    data => {
...
tmptarif(){
  this.authService.listtariftree(this.kdcabang,'RI',this.kdtarif)
  .subscribe(
    data => {
...
caritarif(a){
  this.authService.tarifdetail(this.kdcabang,'2',a.target.value,this.kdbapak,'RI')
  .subscribe(
    data => {
...
this.yangmasuki = 0;
     ...
     klihats(){
      var redirectWindow = window.open(this.URLINVOICE+'clenic/report/pendapatanri.php?kdcabang='+this.kdcabang+'&username='+this.usercetak+'&tgldari='+this.tglp+'&tglsampai='+this.tglpp+'&status='+this.kliniks+'&kdpoli='+this.kliniks, '_blank','location=no,toolbar=no,height='+screen.height+',width='+screen.width+',scrollbars=yes,status=yes');
      redirectWindow.location;
     }
     klihatsx(){
      var redirectWindow = window.open(this.URLINVOICE+'clenic/report/pendapatanriuser.php?kdcabang='+this.kdcabang+'&username='+this.usercetak+'&tgldari='+this.tglp+'&tglsampai='+this.tglpp+'&status='+this.kliniks+'&kdpoli='+this.kliniks, '_blank','location=no,toolbar=no,height='+screen.height+',width='+screen.width+',scrollbars=yes,status=yes');
      redirectWindow.location;
...
nott:any;
 ...
 kcontentpdetail(content,notransaksi,kdproduk){
  ...
  if(kdproduk === '9'){
    ...
    this.authService.trxjualx(this.kdcabang,notransaksi,notransaksi)
    .subscribe(
      data => {
      this.tjual = data
