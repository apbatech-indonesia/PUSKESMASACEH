...
@Component({
  selector: 'app-mutasifarmasi',
  templateUrl: './mutasifarmasi.component.html',
  styles: [
...
export class mutasifarmasiComponent implements OnInit {
...
this.trxmutasix()
  ...
  nmobat:any;
  cariobat(a){
    this.authService.cobatjual(this.kdcabang,this.kdgudang2,a.target.value)
    .subscribe(
      data => {
...
onobat(a){
  ...
  this.authService.obatbykodestok(this.kdcabang,this.nmobat,this.kdgudang2)
  .subscribe(
    data => {
...
trxmutasix(){
  this.authService.trxmutasi(this.kdcabang,this.nomutasi)
  .subscribe(
    data => {
...
onqty(a){
  ...
  // var stoksistem : number;
  // var stokmutasi:number;

  if(parseInt(this.stok) < 0){
    this.toastr.error('STOK KOSONG', 'Eror');
    return;
  }


if(this.nomutasi === ''){
  this.toastr.error('No Mutasi Kosong anda belum create no mutasi atau  belum memilih list mutasi', 'Eror');
}else{

// stoksistem = a.target.value;
// stokmutasi = this.stok;


//   console.log(parseInt(a.target.value));
//   console.log(parseInt(this.stok));



  if(parseInt(a.target.value) > parseInt(this.stok)){
    this.toastr.error('Stok Tidak Cukup', 'Eror');
  }else{
      let body={
    "nomutasi":this.nomutasi,"kdgudangd":this.kdgudang2,"kdgudangke":this.kdgudang,"keterangan":this.keterangan,
    "kdobat":this.nmobat,"qty":parseInt(a.target.value),"kdcabang":this.kdcabang,"stssimpan":'2',"kdklinik":this.kdklinik
  }

  console.log(body)

  this.authService.simpannomutasi(body)
  .subscribe(response => {
...
kdgudang2:any;
    ...
    ubahqty(a,kdobat,nomor,HNA,QTY){
      ...
      "kduser":this.username,"stssimpan":'4',"HNA":HNA,"QTY":QTY}
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          ...
          .subscribe(
           ...
           if(data.length){
            this.toastr.error('Masih Ada Transaksi Silahkan Hapus Dulu List Obatnya', 'Eror');
           }else{
...
let body={"nomutasi":this.nomutasi,"kdcabang":this.kdcabang,"kdklinik":this.kdklinik,"stssimpan":'4'}
    ...
    hapusobat(nomutasi,nomor,kdobat,qty,obat,standart){
      const swalWithBootstrapButtons = Swal.mixin({
        customClass: {
          confirmButton: 'btn btn-success',
          cancelButton: 'btn btn-danger'
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          ...
          let body={"nomutasi":nomutasi,"nomor":nomor,"kdobat":kdobat,"qty":qty,"kdcabang":this.kdcabang,"kdklinik":this.kdklinik,"stssimpan":'3',"kdgudangd":this.kdgudang2,"kdgudangke":this.kdgudang}
  ...
  ktglrad(){
    this.authService.listmutasi(this.kdcabang,this.tglp,this.caripas,'')
    .subscribe(
      data => {
  ...
  this.tbeli = data;
    ...
    caripass(a){
      this.authService.listmutasi(this.kdcabang,this.tglp,this.caripas,a.target.value)
      .subscribe(
        data => {
          this.totalpasenlist = data.length
    this.tbeli = data;
...
pilihpasien(mutasi,tgl,kdgudangd,kdgudangke,keterangan)
  {

    this.tombolcetak= false;
    this.nomutasi = mutasi
    this.tglp = tgl
    this.kdgudang2 =  kdgudangd;
    this.kdgudang = kdgudangke;
...
// }, 200);
  ...
  cetakfaktur(){
    var redirectWindow = window.open(this.URLINVOICE+'clenic/report/buktiretur.php?nomutasi='+this.nomutasi+'&kdcabang='+this.kdcabang, '_blank','location=no,toolbar=no,height=1000,width=1000,scrollbars=yes,status=yes');
