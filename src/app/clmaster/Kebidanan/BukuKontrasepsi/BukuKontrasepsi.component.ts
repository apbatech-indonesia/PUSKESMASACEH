...
export class BukuKontrasepsiComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.cari.tanggal_akhir = this.datePipe.transform(
      this.currentDate,
      "yyyy-MM-dd"
    );
    this.search();
  ...
  update() {
    ...
    this.emrServ.update(body).subscribe((response) => {
  ...
  store() {
    ...
    console.log(body);
    this.emrServ.create(body).subscribe((response) => {
      if (response.success == true) {
  ...
  search() {
    ...
    this.emrServ.list(this.cari).subscribe((resp) => {
  ...
  show(id) {
    this.emrServ.show(id).subscribe((response) => {
      if (response.success == true) {
  ...
  delete(id) {
    ...
    if (isDelete) {
      this.emrServ.delete(id).subscribe((resp) => {
        if (resp.success == true) {
  ...
  searchDokter() {
    this.emrServ.dokterList().subscribe((response) => {
      if (response.success == true) {
  ...
  caripasien(a) {
    this.emrServ.pasienList(a.target.value).subscribe((response) => {
      if (response.success == true) {
  ...
  getPasien() {
    this.emrServ.pasienList("a").subscribe((response) => {
      if (response.success == true) {
        let pasienListOri = response.pasien;
        this.pasienList = pasienListOri.map(function (val, key) {
          return {
